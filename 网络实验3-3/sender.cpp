...
int window_size = 1;                    // 窗口大小为1，单位是数据包的最大大小
        ...
        for (int i = 0; i < receive_num; i++) {
            ...
            if (result > 0) { // 成功接收
                for (int i = 0; i < result; i++) {
                    send_queue.pop();
                    if (Reno_State == Slow_Start) { // 慢启动阶段
                        window_size++; // 设置窗口大小
                    ...
                    else if (Reno_State == Congestion_Avoidance) { // 拥塞避免阶段
                    ...
                    else { // Reno_State == Quick_Recovery // 快速恢复阶段
                        Reno_State = Congestion_Avoidance; // 拥塞避免阶段
                        cout << "发送端进入拥塞避免状态" << endl;
