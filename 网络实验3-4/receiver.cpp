...
int MAX_MIS_COUNT = 1000000;    // 丢包变量
...
int receive_packet(SOCKET& sock, struct sockaddr_in& sender_addr, Packet& pkt_received) {
    ...
    if (pkt_received.seq_num > seq_num_expected) {
        cerr << "期望序号：" << seq_num_expected << "，接收序号：" << pkt_received.seq_num << "，序号不正确，丢弃数据包，并且快速重传" << endl;
        return -1;
    }
...
int handle_packet(SOCKET& sock, struct sockaddr_in& sender_addr, queue<Packet>& receive_queue) {
    ...
    // 特殊情况判断
    if (ret == -1) { // 接收到错误的数据包，快速重传
        // 发送ACK包
        Packet pkt;
        pkt.ACK = true;
        pkt.seq_num = seq_num_share++;
        pkt.ack_num = seq_num_expected - 1;
        send_packet(sock, sender_addr, pkt); // 快速重传
        send_packet(sock, sender_addr, pkt); // 快速重传
        send_packet(sock, sender_addr, pkt); // 快速重传
        return 0;
    }
